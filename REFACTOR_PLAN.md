...
- [ ] Replace `app/src/scripts/trystero-bundle.js` (hand-vendored, 3015 lines) with a real build step. Genuinely not done in this pass — it's on the critical path of P2P viewer connections and needs manual host/viewer verification I can't fully automate, so breaking it into concrete steps here instead of leaving it as one vague item:
  - [ ] Add `esbuild` as a devDependency (fast, zero-config for bundling a single entry point — no need for a heavier bundler here).
  - [ ] Add a script, e.g. `npm run build:trystero`, that bundles `@trystero-p2p/torrent` (which pulls in `@trystero-p2p/core` via a bare specifier the browser can't resolve unbundled) into one browser-consumable ESM file.
  - [ ] Diff the generated output's exported surface against the current hand-vendored `trystero-bundle.js` (at minimum: confirm `joinRoom` and whatever else `p2p-signaler.js` imports are present and behave the same) before wiring it in.
  - [ ] Point `p2p-signaler.js`'s `import { joinRoom } from './trystero-bundle.js'` at the generated file, and delete the hand-vendored copy.
  - [ ] Manually verify end-to-end: run the app, host a session, join as a viewer, confirm the P2P/WebRTC signaling path still connects — do not rely on lint/unit tests alone for this step, they don't exercise the actual WebRTC handshake.
  - [ ] Ship this as its own isolated PR/commit, separate from any other Phase 2 work, so a regression here is trivial to bisect and revert.
