...
function setDesktopVolume(val) {
  ...
  if (!window._masterMuteActive && _desktopGainNode) _desktopGainNode.gain.value = v / 100;
}
...
function setHostMicGain(val) {
  ...
  if (!window._masterMuteActive && _hostMicGainNode) _hostMicGainNode.gain.value = v / 100;
}
...
function attachDesktopGain(stream) {
  ...
  try {
    ...
    console.warn('[HostAudio] Gain node failed:', e);
  }
}
...
function toggleMasterMute() {
  ...
  if (active) {
    ...
    if (btn) {
      btn.classList.add('master-mic-kill');
      btn.title = 'Audio MUTED — click to restore';
    }
    if (icon) {
      icon.src = '/assets/icons/speaker-off.svg';
      icon.style.filter = 'invert(0.4) sepia(1) saturate(6) hue-rotate(-20deg)';
    }
    ...
    if (btn) {
      btn.classList.remove('master-mic-kill');
      btn.title = 'Cut Desktop Audio to Stream';
    }
    if (icon) {
      icon.src = '/assets/icons/speaker.svg';
      icon.style.filter = 'invert(0.6)';
    }
...
function startAudioMeter(stream) {
  ...
  (function draw() {
    ...
    fill.style.width = Math.min(100, (data.reduce((a, b) => a + b, 0) / data.length) * 2) + '%';
  })();
}
...
function disableMic() {
  ...
  if (micSender && pc && pc.signalingState !== 'closed') {
    try {
      ...
      console.warn('[Mic] removeTrack error:', e);
    }
    micSender = null;
  }
  if (localMicStream) {
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (sv) {
    sv.value = Math.round(_audioPrefs.streamVol * 100);
    const d = document.getElementById('streamVolVal');
    if (d) d.textContent = sv.value;
  }
  if (sg) {
    sg.value = Math.round(_audioPrefs.micGain * 100);
    const d = document.getElementById('micGainVal');
    if (d) d.textContent = sg.value;
  }
  if (sm) {
    sm.value = Math.round(_audioPrefs.selfMonitor * 100);
    const d = document.getElementById('selfMonitorVal');
    if (d) d.textContent = sm.value;
  }
  if (ov) {
    ov.value = Math.round(_audioPrefs.othersVol * 100);
    const d = document.getElementById('othersVolVal');
    if (d) d.textContent = ov.value;
  }
...
function setStreamVolume(val) {
  ...
  if (v > 0 && audioMuted) {
    ...
    if (videoEl?.srcObject)
      videoEl.srcObject.getAudioTracks().forEach((t) => {
        t.enabled = true;
      });
    const remoteAudioEl = document.getElementById('remote-audio');
...
function teardownSelfMonitor() {
  if (selfMonitorSrc) {
    try {
      selfMonitorSrc.disconnect();
    } catch {}
    selfMonitorSrc = null;
  }
  if (selfMonitorGain) {
    try {
      selfMonitorGain.disconnect();
    } catch {}
