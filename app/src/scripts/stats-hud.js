...
function _updateStatsHud() {
  ...
  _elText(
    'hudPipeline',
    pipe ? (pipe.value === 'custom_webcodecs' ? 'WC (GL)' : pipe.value === 'webcodecs' ? 'WC' : 'WebRTC') : '—'
  );
    ...
    for (const r of stats.values()) {
      ...
      if (r.type === 'inbound-rtp' && r.kind === 'video') {
        ...
        if (prevStatsTime) {
          ...
          kbps = (((r.bytesReceived - prevBytesReceived) * 8) / ((r.timestamp - prevStatsTime) / 1000) / 1000).toFixed(
            0
          );
        }
        prevBytesReceived = r.bytesReceived;
        prevStatsTime = r.timestamp;
        prevJitterDelay = r.jitterBufferDelay || 0;
        prevEmitted = r.jitterBufferEmittedCount || 1;
...
function _renderStatsHudBars(rtt, jitterBufMs, kbps, packetsLost, packetsReceived) {
  ...
  let bars, colour;
  if (rttN < 40 && lossRatio < 1) {
    bars = '▪▪▪▪';
    colour = '#4ade80';
  } // excellent — green
  else if (rttN < 80 && lossRatio < 3) {
    bars = '▪▪▪○';
    colour = '#a3e635';
  } // good — lime
  else if (rttN < 140 && lossRatio < 6) {
    bars = '▪▪○○';
    colour = '#facc15';
  } // fair — yellow
  else if (rttN < 220 && lossRatio < 12) {
    bars = '▪○○○';
    colour = '#fb923c';
  } // poor — orange
  else {
...
function _renderNetStatsOverlay({
  rtt,
  sawInboundVideo,
  codecName,
  kbps,
