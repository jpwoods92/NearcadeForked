...
function _micSvg(state) {
  ...
  const style =
    state === 1
      ...
      : state >= 2
        ? 'filter:invert(0.4) sepia(1) saturate(6) hue-rotate(-20deg);'
        : 'filter:invert(0.75);';
  return `<img src="/assets/icons/${icon}.svg" style="width:14px;height:14px;flex-shrink:0;display:block;${style}" alt="">`;
}

const _micTitles = ['Mic Normal (100%)', 'Mic Quiet (50%)', 'Locally Muted', 'Globally Muted'];
...
function renderRoster(list) {
  ...
  controllers.forEach((v, index) => {
    ...
    r.innerHTML = `
        ...
        ${
          v.id === 'host_0'
            ? `<span style="font-size:9px;color:var(--muted);">Host</span>`
            : `
...
function toggleGlobalMicKill() {
  ...
  log(
    _globalMicKillActive ? 'Global mic kill: all viewer mics disabled' : 'Global mic kill lifted: viewer mics restored',
...
function changeInputMode(viewerId, newMode, viewerName) {
  if (ws && ws.readyState === 1) {
    ...
    log(
      I18N.t('Input mode for viewer ${viewerId} set to ${newMode}')
        .replace('${viewerId}', viewerId)
        .replace('${newMode}', newMode),
      'ok'
    );
...
function updateSlotsAfterDrop(container) {
  Array.from(container.querySelectorAll('.rcard')).forEach((item, index) => {
    if (ws && ws.readyState === 1)
      ws.send(JSON.stringify({ type: 'assign-slot', viewerId: item.dataset.id, slot: index }));
  });
}
...
function _refreshViewerPanel() {
  ...
  if (!viewers.length) {
    list.innerHTML =
      '<div style="font-size:10px;color:var(--muted2);padding:12px;text-align:center;">No viewers connected</div>';
    return;
  }
  viewers.forEach((v) => {
...
if (typeof module !== 'undefined' && module.exports) {
  module.exports = {
    ...
    changeInputMode,
    attachDragDrop,
    updateSlotsAfterDrop,
    kickViewer,
    toggleSlotLock,
    toggleViewerPanel,
    _refreshViewerPanel,
    toggleViewerInputPerm,
  };
}
