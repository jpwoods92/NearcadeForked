...
class CaptureManager {
  ...
  getStatus() {
    return {
      ...
      details:
        this._activeMethod === 'ffmpeg'
          ? `FFmpeg via ${this._ffmpegEncoder}`
          : this._activeMethod === 'pipewire'
            ? `PipeWire node: ${this._pipewireNodeName || 'auto'}`
            : this._activeMethod === 'wivrn'
              ? `WiVRn stream: ${this._wivrnPort || 'not started'}`
              : this._activeMethod === 'gstreamer_webrtc'
                ? 'Native C++ GStreamer WebRTC'
                : 'Frontend Execution',
    };
  }
  ...
  async _startGstWebRTC(options) {
    ...
    try {
      ...
      if (targetNode) {
        ...
        console.log(
          `[CaptureManager] Auto-discovered PipeWire target node: ${targetNode.name} (ID: ${targetNode.id}, Serial: ${targetNode.serial}) -> Passing ${targetStr}`
        );
        args.push('--node', targetStr);
      } else {
        console.warn(
          '[CaptureManager] No headless PipeWire node found. GStreamer will fallback to default video source.'
        );
      }
    } catch (err) {
  ...
  // Pass the routing function so CaptureManager can emit WebRTC offers to server/ws.js
  setGstSignalingCallback(cb) {
    this._gstSignalingCallback = cb;
  }

  // Send Answers / ICE candidates from server/ws.js into the Python daemon
