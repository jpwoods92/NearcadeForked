// DXGI Desktop Duplication capture addon (Windows).
// Captures the desktop framebuffer directly from the GPU, bypassing DWM
// compositor. N-API interface: startCapture(monitorIndex) -> { getFrame() -> Buffer }
//
// Ported from upstream (src/sidecar/capture/capture-win.cc). Source-only:
// this sandbox has no Windows SDK/MSVC to compile or run it against a real
// GPU/display, so it hasn't been build- or runtime-verified here. Needs a
// real Windows build + a CaptureManager.js 'dxgi' method (see that file's
// existing 'ffmpeg'/'pipewire'/'wivrn'/'webcodecs'/'webrtc' cases) before
