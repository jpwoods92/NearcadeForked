...
function send(msg) {
    ...
    if (msg.type === 'gamepad') {
        ...
        console.log(`[DEBUG GP] pad_id="${validated.pad_id}" bridge=${!!state.bridge} python=${!!state.pythonProc} hybrid=${state.hybridInputEnabled}`);
    ...
    if ((!state.bridge || state.hybridInputEnabled) && state.pythonProc && state.pythonProc.stdin.writable) {
        try { state.pythonProc.stdin.write(JSON.stringify(validated) + '\n'); } catch (e) { }
...
module.exports = {
    ...
    events: state.events,
    getViewerForSlot,
    get _bridge() { return state.bridge; },
};
